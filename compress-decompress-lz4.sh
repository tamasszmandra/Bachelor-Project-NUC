...
for input_file in "$input_directory"/*; do
    ...
    for ((i=1; i<=15; i++)); do
        ...
        for level in {1..9}; do
            ...
            # Lz4
            start=$(date +%s.%N)
            lz4 -"$level" -c "$input_file" > "${filename_no_extension}_$level.lz4" 2>> "$filename_no_extension-total.txt"
            end=$(date +%s.%N)
            ...
            compressed_size=$(du -b "${filename_no_extension}_$level.lz4" | cut -f1)
    ...
    # Uncompress .lz4 file write to .txt file
    start=$(date +%s.%N)
    ...
    # Uncompression Ratio
    compressed_size=$(du -b "${filename_no_extension}_1.lz4" | cut -f1)
...
echo " "
echo "Writing file totalresult-lz4.txt...done"
cat *.txt > totalresult-lz4.txt
