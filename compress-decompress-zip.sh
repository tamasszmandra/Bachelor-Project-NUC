...
for input_file in "$input_directory"/*; do
    ...
    for ((i=1; i<=15; i++)); do
        ...
        for level in {1..9}; do
            ...
            # Zip
            start=$(date +%s.%N)
            zip -"$level" -q "${filename_no_extension}_$level.zip" "$input_file" | tee -a "$filename_no_extension-total.txt"
            end=$(date +%s.%N)
            ...
            compressed_size=$(du -b "${filename_no_extension}_$level.zip" | cut -f1)
    ...
    #echo "Writing all results to allresults-zip.txt file."
    echo " "
    ...
    # Uncompress .zip file write to .txt file
    start=$(date +%s.%N)
    ...
    # Uncompression Ratio
    compressed_size=$(du -b "${filename_no_extension}_1.zip" | cut -f1)
    ...
    echo "---------------------------------------------------------------------------"
    echo " "
    echo "Uncompressing $filename_no_extension.zip...done."
    echo "Writing uncompression details to $filename_no_extension-uncompressed.txt"
    echo " "
done
echo " "
echo "Writing file to totalresult-zip.txt...done."
cat *.txt > totalresult-zip.txt
