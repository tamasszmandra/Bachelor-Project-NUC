...
for input_file in "$input_directory"/*; do
    ...
    for ((i=1; i<=15; i++)); do
        ...
        for level in {1..9}; do
            ...
            # Bzip2
            start=$(date +%s.%N)
            bzip2 -"$level" -c "$input_file" > "$filename_no_extension.bz2"
            end=$(date +%s.%N)
            ...
            compressed_size=$(du -b "$filename_no_extension.bz2" | cut -f1)
    ...
    #echo "Writing all results to allresults-bzip2.txt file."
    echo " "

    # Write file size to file size list
    compressed_sizes=""
    for level in {1..9}; do
        compressed_size=$(du -b "${filename_no_extension}_$level.bz2" | cut -f1)
    ...
    start=$(date +%s.%N)
    bzip2 -d -k "$filename_no_extension.bz2" | tee -a "$filename_no_extension.bz2"
    end=$(date +%s.%N)
    ...
    echo "---------------------------------------------------------------------------"
    echo " "
    echo "Uncompressing $filename_no_extension.bz2...done."
    echo "Writing uncompression details to $filename_no_extension-uncompressed.txt"
    echo " "
done
echo " "
echo "Writing results to file totalresult-bzip2.txt...done."
cat *.txt > totalresult-bzip2.txt
